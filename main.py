...
# Helper to truncate at sentence end
...
# Rate-limited mention lookup
def safe_mention_lookup(fn, *args, **kwargs):
    now = time.time()
    if mentions_timestamps and now - mentions_timestamps[0] > RATE_WINDOW:
        mentions_timestamps.popleft()
    if len(mentions_timestamps) >= MENTIONS_LIMIT:
        time.sleep(RATE_WINDOW - (now - mentions_timestamps[0]) + 1)
    res = fn(*args, **kwargs)
    mentions_timestamps.append(time.time())
    return res

# Rate-limited tweet with community support

def safe_tweet(text: str, media_id=None, **kwargs):
    now = time.time()
    if tweet_timestamps and now - tweet_timestamps[0] > RATE_WINDOW:
        tweet_timestamps.popleft()
    if len(tweet_timestamps) >= TWEETS_LIMIT:
        time.sleep(RATE_WINDOW - (now - tweet_timestamps[0]) + 1)
    params = {"community_id": COMMUNITY_ID}
    if media_id:
        params["media_ids"] = [media_id]
    resp = x_client.create_tweet(text=text, **params, **kwargs)
...
async def post_raid(tweet):
    ...
    prompt = (
        f"History:{history}\n"
        f"User: '{tweet.text}'\n"
        "Write a one-liner bullpost for $DEGEN based on above. "
...
async def mention_loop():
    while True:
        try:
            ...
            res = safe_mention_lookup(x_client.get_users_mentions, **params)
