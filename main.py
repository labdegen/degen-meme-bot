# main.py — Run this with `python main.py` as a Render Worker
...
# Twitter “since_id” workaround
current_time_ms = int(time.time()*1000) - 1_728_000_000
...
def lookup_address(token: str) -> str:
    ...
    for itm in toks:
        if itm.get("symbol","").lower()==t.lower():
            return itm.get("contractAddress")
...
async def safe_mention_lookup(*args, **kw):
    return await safe_api_call(*args, mentions_q, MENTIONS_LIMIT, **kw)

async def safe_search(fn, *args, **kw):
    return await safe_api_call(fn, search_q, SEARCH_LIMIT, *args, **kw)

async def safe_tweet(text: str, media_id=None, **kw):
    def _post(t, m, **kw2):
        return x_client.create_tweet(text=t, media_ids=[m] if m else None, **kw2)
    return await safe_api_call(_post, tweets_q, TWEETS_LIMIT, text, media_id, **kw)

# Non-blocking wrappers for blocking helpers
...
async def post_raid(tweet):
    ...
    hist = get_thread_history(cid)
    p = (f"History:{hist}\nUser: '{tweet.text}'\n"
         f"Write a one-liner bullpost for $DEGEN… End with NFA.")
    msg = await ask_grok_async(p)
...
async def handle_mention(tw):
    ...
    if re.match(r"^\s*dex\s*$", txt, re.IGNORECASE):
        await safe_tweet(
            format_metrics(await fetch_data_async(DEGEN_ADDR)) + DEGEN_ADDR,
    ...
    if token:
        ...
        if addr:
            await safe_tweet(
                format_metrics(await fetch_data_async(addr)) + addr,
                in_reply_to_tweet_id=tw.id
            )
            redis_client.sadd(f"{REDIS_PREFIX}replied_ids", str(tw.id))
            return
    ...
    prompt = (f"History:{hist}\nUser asked: \"{txt}\"\n"
              "Answer concisely, then in gambler style mention stacking $DEGEN. End with NFA.")
...
async def mention_loop():
    while True:
        try:
            ...
            params = {"id": BOT_ID, "tweet_fields":["id","text","conversation_id"], "max_results":10}
            ...
            logger.error("Mention loop error:", exc_info=True)
...
async def search_mentions_loop():
    ...
    while True:
        try:
            ...
            res = await safe_search(x_client.search_recent_tweets, query=query, tweet_fields=["id","text","conversation_id"], max_results=10)
            if res and res.data:
            ...
            logger.error("Search loop error:", exc_info=True)
...
async def hourly_post_loop():
    while True:
        try:
            ...
            logger.error("Hourly post error:", exc_info=True)
